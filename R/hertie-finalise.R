...
#' Arrange alignment and save Hertie ggplot chart
#'
...
#'  which needs to be a PNG file - defaults to Hertie blocks image that sits within the data folder of your package
...
#'
#' @export
hertie_finalise_plot <- function(plot_name,
                              source_name,
  ...
  if(hertie_logo == T){
    logo_image_path1 = file.path(path.package("heRtie"),"data/pics/hertie-logo.png")
  }else{
    logo_image_path1 = file.path(path.package("heRtie"),"data/pics/hertie-logo.png")
  }


  footer <- create_hertie_footer(source_name, logo_image_path1,footer_start,fontsize = fontsize)
